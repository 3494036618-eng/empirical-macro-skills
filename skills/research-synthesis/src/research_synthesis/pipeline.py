...
def _readiness_errors(envelopes: EnvelopeMap) -> list[str]:
    envelope = envelopes["research_design"]
    design = envelope.statuses
    errors = []
    reviewed_causal_candidate = (
        design.get("execution_status") == "partial"
        and design.get("design_readiness") == "review_required"
        and design.get("review_required") is True
        and envelope.claim_eligibility == "causal_candidate"
    )
    if design.get("execution_status") != "success" and not reviewed_causal_candidate:
